...
class Connection(object):
    ...
    def _build_url(self, path='', query=None):
        ...
        `path` \n
        The path that will be added to the base URL. The default is an
        empty string.

        `query` \n
        A dictionary containing the query parameters to add to the
    ...
    def _check_content_type(self, expected_re, expected_name, headers):
        ...
        `expected_re` \n
        The regular expression used to check the expected
        content type.

        `expected_name` \n
        The name of the expected content type.
    ...
    def _read_reponse(self, context):
        ...
        `context` \n
        tuple which contains cur easy, response body,
